...
public class VentaDAO implements IVentaDAO {
    ...
    private static final String INSERT_VENTA
            = "INSERT INTO public.ventas (fecha, hora, id_cliente, subtotal, total) "
            + "VALUES (?, ?, ?, ?, ?)";

    private static final String SELECT_ALL_VENTAS
            = "SELECT v.id_ventas, v.fecha, v.hora, v.id_cliente, v.subtotal, v.total, "
            + "c.nombre AS nombre_cliente, c.apellido AS apellido_cliente "
            + "FROM public.ventas v "
            + "JOIN public.cliente c ON v.id_cliente = c.id_cliente "
            + "ORDER BY v.fecha, v.id_ventas";

    private static final String SELECT_VENTA_ID
            = "SELECT v.id_ventas, v.fecha, v.hora, v.id_cliente, v.subtotal, v.total "
            + "FROM public.ventas v "
            + "WHERE v.id_ventas = ?";

    private static final String DELETE_VENTA
            = "DELETE FROM public.ventas WHERE id_ventas = ?";

    private static final String INSERT_DETALLE
            = "INSERT INTO public.detalle_venta (id_venta, id_producto, cantidad, precio, subtotal) "
            + "VALUES (?, ?, ?, ?, ?)";

    private static final String SELECT_DETALLES_POR_VENTA
            = "SELECT dv.id_detalleventas, dv.id_venta, dv.id_producto, "
    ...
    private static final String DELETE_DETALLES_POR_VENTA
            = "DELETE FROM public.detalle_venta WHERE id_venta = ?";

    private static final String RESTAR_STOCK
            = "UPDATE public.productos SET stock = stock - ? WHERE id_productos = ?";

    private static final String DEVOLVER_STOCK
            = "UPDATE public.productos SET stock = stock + ? WHERE id_productos = ?";

    private static final String RESTAR_STOCK_INVENTARIO
            = "UPDATE public.inventario SET stock_disponible = stock_disponible - ? WHERE id_producto = ?";

    private static final String DEVOLVER_STOCK_INVENTARIO
            = "UPDATE public.inventario SET stock_disponible = stock_disponible + ? WHERE id_producto = ?";
